...
ByteCopy(gsl::span<std::byte> destination, T const source) {
  uint32_t shift{0};

  for (std::ptrdiff_t index{0};
       index < sizeof(T) && index <= destination.size(); ++index, shift += 8) {
...
CreateType(gsl::span<uint8_t const> const source) {
  uint32_t shift{0};
  T tmp_destination{0};

  for (std::ptrdiff_t index{sizeof(T) - 1};
       index >= 0 && index <= source.size(); --index, shift += 8) {
